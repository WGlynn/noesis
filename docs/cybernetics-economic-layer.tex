...
and the graded objectivity dial. \emph{Data-blocked}: the learned, retraining value evaluator, whose strength is empirical rather than
deductive. Its first real-data test came back null: on real outcome labels the learned measure did not
reliably beat a fixed structural proxy, so the adaptive, un-gameable property is unsupported (not
refuted) and remains open pending the faithful provenance-feature port. The claim of this paper is not
that the loop is fully closed in code today. It is that the static component whose absence defeated
every prior attempt, the honest sensor, is built and tested, and that once the adaptive layer is
established the governor and the effectors are engineering rather than open problems.
