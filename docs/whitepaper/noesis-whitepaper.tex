...
\texttt{noesis}}
\date{Version 2.2 \quad June 2026}
...
\begin{abstract}
\noindent
Bitcoin rewards exactly one thing: burning electricity. Not good work. Not useful work. Just
electricity. Whoever burns the most, wins. It never asks whether you created anything worth having.
This paper specifies a chain that does, and the claim is larger than a better proof of work: measuring
contribution on-chain unlocks two things no possession chain like Bitcoin can reach.
...
value function, and credited on one canonical graph without loss. A possession chain cannot do this;
its value is priced off-chain, so there is nothing to carry across a merge, and Bitcoin has no organ
for valuing another chain's work. A value chain has exactly that organ. So rival useful-work chains
need not be beaten; they converge in by reverse fork, and the standards war over which chain wins
dissolves into all of them, on one attribution graph. To every other chain, the offer stops being
\emph{come compete} and becomes \emph{come join us}.
...
\emph{owned and transferred} like a UTXO, and used to \emph{secure consensus}, in place of a proof of
wasted energy. The one hard problem is measuring what a contribution is really worth when no market
...
Proof-of-work secures \emph{order}, not \emph{value}. A miner proves it burned energy. The network
agrees on an order for the transactions. What the coin is worth is decided somewhere else, by a market
the chain never sees. Nowhere does the chain record \emph{who created what value}. The common belief
that miners earn value by doing work is a category error: the work is arbitrary, and the value comes
from outside.
