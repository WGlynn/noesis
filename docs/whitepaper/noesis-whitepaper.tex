...
\texttt{noesis}}
\date{Version 2.4 \quad June 2026}
...
\begin{abstract}
\noindent
Bitcoin proved that strangers can agree on who owns what, with no one in charge. That was the mastery
of a new fire: the first control of a force no one had governed before, and the ground every chain
after it stands on. What it really did was bring into being a market that had no medium to exist in
before, the market for trustless ownership. This is what cryptoeconomics does: it mints the medium a
market was missing, and the market appears. Bitcoin gave possession its medium and left the coin's
value to an outside market, which was the right design for what it was. This paper mints the next
medium, for the one market still without one, contribution itself: it measures the value of a
contribution on-chain and makes that measurement the object consensus is about, not who holds the coin
but how much each participant actually contributed. The claim is larger than a better proof of work,
because measuring contribution endogenously unlocks two things a possession chain cannot reach.
...
storage; you cannot buy consensus. Cooperation becomes the individually rational strategy because the
mechanism conserves value when you cooperate and fragments it when you do not, not because anyone
promised to play fair.
...
owns what. Everything in this paper is built on that achievement. It secured \emph{order}: a miner
expends energy, the network agrees on a sequence, and the worth of the coin is settled elsewhere, by a
market the chain never sees. What it does not record is \emph{who created what value}, and it did not
need to. This is not a flaw to correct but a stage to build on. Mastering fire did not require
understanding combustion, and it was still the step without which there are no engines and no rockets.
Bitcoin is the mastery of agreement over possession; what comes next is agreement over contribution.
...
stake, the consensus weight, is a functional of $v$. The obvious choices of $v$ are either trivial or
gameable; this section gives a $v$ that is neither.
