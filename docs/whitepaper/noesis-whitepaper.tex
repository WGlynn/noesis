...
\texttt{noesis}}
\date{Version 2.9 \quad June 2026}
...
instructively. If the coalition value is additive, $v(S)=\sum_{i\in S} v(\{i\})$, then the Shapley
...
\end{equation}
where $S/G$ are the connected components of $S$ in $G$. So a block's credit is its average added value
over every order in which the blocks could have been assembled, counting only the groupings actually
wired together by provenance. Value flows along provenance edges, not over arbitrary sets; plain
Shapley is the special case of a complete graph. Exact computation is exponential, so we estimate
$\phi_i$ by Monte-Carlo permutation sampling (Data-Shapley), Shapley specialized to data valuation. In
a working prototype over real blocks, replacing the additive game with a submodular coverage
outcome-value made the Myerson values diverge materially from the proportional baseline, mean absolute
difference $\lVert \phi - \text{Copeland}\rVert_1 \approx 0.26$: the cooperative value became
load-bearing, rewarding pivotal blocks and discounting redundant ones.
...
The mechanisms above have a single economic reading. Across a decade, decentralized systems tried
libertarian permissionlessness (which produced extractable value and front-running), regulated
clearinghouses (which produced exclusion), commons-based production (which produced mining and validator
cartels), and token governance (which produced capture). Each correctly rejected its predecessor and
reproduced extraction by a new route. The common error was to locate cooperation in a \emph{norm}, a
\emph{regulator}, or a \emph{vote}, all of them defectable, rather than in the mechanism itself. The
