...
\texttt{noesis}}
\date{Version 3.2 \quad June 2026}
...
is usefulness by construction. The money layer is a \emph{proportional} proof-of-work currency in the
manner of Ergon: issuance scales with the work committed, so the coin is demand-elastic and targets
stable purchasing power rather than appreciation. A coin not expected to rise in value is one meant to be
\emph{spent}, and the proportional arms race makes this self-enforcing: miners must keep selling to cover
ongoing power and hardware costs, so hoarding a non-appreciating coin is bad business and the coin
circulates as a true medium of exchange. Its work then buys two things at once, Sybil-resistant security
and a stable unit of account, and its usefulness speaks for itself without bolting a foreign task
(primes, protein folds) onto the hash. Proportional proof-of-work is also the right home for both work
and price stability: its economics make the conditions for an \emph{energy money}, a coin anchored to
the real cost of energy and therefore stable and spendable. This is what proof-of-work does best: it is
an \emph{energy oracle}, pricing real-world energy into money. Using proof-of-work here is therefore not
a concession but a strength, each tool placed where it fits, cognition (PoM) securing value and energy
pricing money. This layer is \emph{designed, not yet built}; the reference
stub's current issuance curve is a placeholder to be replaced with the proportional rule.
