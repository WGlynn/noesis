...
\texttt{noesis}}
\date{Version 1.4 \quad June 2026}
...
elicit contribution honestly, an organ Bitcoin does not have, which is why valuation is exported to
an external market it never observes and the gap fills with extraction. This paper specifies the
missing organ: a chain on which value is \emph{created} as units of contribution,
...
Put simply: the chain pays you only for verified, non-duplicate contribution, measures how much your
work actually mattered by how everything else builds on it, and lets that earned standing, not
bought stake, secure the network. The mechanisms reported here are implemented and tested in an
open reference node; we mark throughout what is demonstrated and what is designed.
\end{abstract}
...
proof-of-\emph{useful}-work (information as the work --- value measured by the signal a contribution
carries, not the energy it burns), then \textbf{proof of mind}, a proof of
...
evaluator for $v$. In plain terms: from many ``$A$ contributed more than $B$'' judgments we fit a
single ``how much it added'' score per block, the same way a chess rating or an RLHF reward model is
learned from pairwise wins.
...
\end{equation}
where $S/G$ are the connected components of $S$ in $G$. In plain terms: a block's credit is its
average added value taken over every order in which the blocks could have been assembled, counting
only the groupings actually wired together by provenance. Value flows along provenance edges, not over
...
\end{equation}
In plain terms: you are paid only for what you add that no earlier block already covered.
