...
## THIRD PASS (RI-003, 2026-06-19) — ‼ FOUND REAL PRE-EMPTIONS (first pass to cut against us)

Deep-research pass 3 (105 agents, 24/25 verified) targeted the 2 claims passes 1-2 left open. Both took hits.

- **(e) Hodge-residual-as-manipulation-alarm ⇒ PARTIAL-PRIOR-ART / leaning REINVENTING.** **DPoR — "Delegated Proof of Reputation" (arXiv:1912.04065, 2019)** explicitly proposes a Hodge-decomposition filter that **freezes/confiscates stake** on nodes whose token-flows lie in the **harmonic/curl subspaces**, framed as **sybil/boosting resistance + validation certificate**. Nearly the exact construct. CAVEATS (the surviving differentiator): DPoR's slashing is a **PROPOSAL** ("a suggestion," no formula/algorithm); DPoR ranks **raw token-transfer flows** for reputation, NOT a learned value scalar. JLYY (2011) founds residual-as-certificate but only for INTRINSIC (Arrow/Condorcet) inconsistency, no adversarial framing. Ma et al. (2023) = HodgeRank as VICTIM, not detector. **⇒ §9 "as far as we are aware not prior" is NOW FALSE — must correct: cite DPoR, narrow (e) to "learned value-model + endogenous-value-as-consensus" pairing (which DPoR lacks).**
- **(a) ⇒ SURVIVES decentralized-TRAINING (Gensyn/Verde, Prime Intellect/TOPLOC, Nous/Psyche = verified-compute/honest-execution, NOT value) but DOES NOT SURVIVE decentralized-INFERENCE.** **Bittensor Yuma (live 2021)** + **Fortytwo (arXiv:2510.24801, Oct-2025)** make endogenous OUTPUT QUALITY the reward/stake/consensus object. ‼ Fortytwo uses "3N pseudo-random pairwise comparisons aggregated via an improved **Bradley-Terry** model; highest-quality response wins consensus; reputation evolves from response-quality" — STRIKINGLY close to our elicitation layer + uses a PoM-adjacent framing (the agent even mislabeled it "the Noesis whitepaper's own companion"). **READ FORTYTWO DIRECTLY (Will).**
- **DEFENSIBLE DIFFERENTIATOR (defend-frame ¬ concede):** Yuma + Fortytwo use **subjective PEER-RANKED / validator-AGREEMENT** quality, NOT (i) a **cooperative-game synergy value** (Myerson over a provenance DAG), (ii) a **learned value-MODEL bound to a residual trust-certificate**, or (iii) measured value as the **on-chain finality/stake object directly** (Fortytwo's reputation weights consensus but is not itself the finality object). The bare "output-value-as-consensus-object" claim is pre-empted; the PRECISE Noesis mechanism (objective synergy-measurement + learned-model + value-as-finality) is differentiated. Narrow (a), don't drop it.
- **MOST DANGEROUS COLLISION:** Bittensor Yuma for (a)-inference; DPoR for (e). **WILL-DECISION PENDING** on framing (defend-hard vs concede-and-narrow) before §9 rewrite.
- **Pass-3 sources:** DPoR arXiv:1912.04065 · JLYY arXiv:0811.1067 · Ma et al. arXiv:2209.05742 · robust-HodgeRank arXiv:1408.3467 · Fortytwo arXiv:2510.24801 · Gensyn/Verde · Prime Intellect/TOPLOC.
