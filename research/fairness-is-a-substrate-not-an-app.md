...
## The extractive base eats the fair island

Now the mechanism, the part that turns this from a complaint into a design law. It would be comforting to think that fairness fails to appear only because no one has funded it, a simple case of a public good going unprovided. That is too gentle. The truth is more active: an extractive economy does not merely fail to build fairness, it cannibalizes any isolated attempt to add it.

Part 1's own example is the proof, and it was hiding in plain sight. The neutral, MEV-resistant batch-auction venue exists and works. But it cannot stand on its own in an extractive sea. It survives only by sourcing its liquidity from the very extractive base it was built to escape, which turns it from a replacement for that base into a skin stretched over it. The fair island did not defeat the extractive ocean. The ocean absorbed it and kept the island alive as a feature of itself. Ordinary frictions, distribution and habit and the cost of switching, would have slowed the fair venue on their own. Cannibalization is the part that is worse than that: not just a public good going unfunded, but a market digesting the antibody it was given.

So there is a sharp claim here, and I want to state it at exactly the strength it can carry and no more. It is not that fairness is all-or-nothing everywhere. Partial substrates exist, bounded domains exist, hybrid layers with credible guarantees exist. The claim is narrower and harder to dodge: fairness cannot survive as a bolt-on when the underlying substrate systematically rewards extraction. In that condition, and it is the condition crypto is actually in, the middle option is the bolt-on, and the bolt-on is the thing that gets cannibalized. So the real choice is not cheap fairness versus expensive fairness. It is paying the bounded cost once, at the base, to change what the whole structure rewards, versus building fairness on top of a base that pays to eat it and watching it get eaten.

## Fairness has to be the substrate

That reframes the entire problem, and it is the thesis of Part 2. Fairness cannot be an application. It cannot be a better DEX, a fairer protocol, a neutral venue running on top of an extractive chain, because the substrate underneath sets the physics that the application lives inside, and if the substrate pays for extraction then the application either conforms or gets eaten. You cannot out-app an extractive base. The only place a positive-sum rule can survive is the base itself, where it defines the physics rather than fighting them.
