...
- [ ] CONTINUE.md — - **DECIDED (cf31d23) → BUILT.** `resolve_refuted_guarded` no longer gates the whole settlement on one `defendant_id`; it judges EACH certifier on its OWN standing. New param `certifier_keys: &[(u64, Vec<u8>)]` (the same key↔id join idiom a...
