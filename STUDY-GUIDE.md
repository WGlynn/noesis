...
3. [ ] **ROADMAP.md** — - **2026-06-19 (x)** — BUILT (pom-roadmap-advance fire): the **lock-sig step-2 inert shape** — `TokenTx::spend_is_authorized(input, auth, tx_digest)` (`node/src/runtime.rs`), the deploy-independent grain after the (w) `tx_digest` serializer...
