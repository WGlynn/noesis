...
- [ ] CONTINUE.md — - **BUILT ✅** closed a runtime gaming vector: the **paraphrase-padding sybil ring**. The live PoM path (`runtime.rs::apply` → `pom_scores` → plain `temporal_novelty`) was NOT using the near-dup similarity floor
...
- [ ] README.md — Other chains compete winner-take-all to be the money, the standard, the one that wins. Builders pick sides. Communities split in forks. Rivals claw at the same liquidity and attention. Underneath the "we are all cooperative" branding, the b...
