...
3. [ ] **ROADMAP.md** — - **2026-06-15** — DESIGN tick (no code; PCP-gate at ~330k session tokens, AND a full moat BUILD already shipped this session — the asymmetric-appeal guard below; a second delicate settlement-code build in the same context is exactly what t...
...
- [ ] CONTINUE.md — - **DECIDED→DEMONSTRATED.** Built `dispute::appeal_refutes_asymmetric` (§7.1c-guard): the PoM-minimized appeal court may only MONOTONE-DECREASE a down-weighted-dimension DEFENDANT's conviction (`pre_appeal && appeal`), a one-way ratchet tow...
