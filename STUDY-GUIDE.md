...
3. [ ] **ROADMAP.md** — - **2026-06-18 (r)** — 🔬 OPEN GAP NAMED — multi-identity volume split defeats the (q) per-identity damping. The (q) λ^r fix caps ONE identity's volume, but an attacker splitting the same volume across K DISTINCT VESTED identities posts one ...
