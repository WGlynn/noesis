...
3. [ ] **ROADMAP.md** — - **2026-06-17 (l)** — DESIGN tick (no code; PCP-gate — a FRESH session at ~250k context after an unrelated heavy build hour; delicate state-transition surgery on the moat belongs in a low-context window, exactly what the gate guards). Adva...
...
- [ ] CONTINUE.md — - **DESIGN tick (no code; PCP-gate — (j) shipped a delicate moat build this session @ ~246k).** Advances the (j) NEXT target (on-VM single-use) named→DECIDED. - **DECISION:** on-VM single-use = committed-UTXO-set membership + rolling-root r...
