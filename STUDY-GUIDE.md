...
3. [ ] **ROADMAP.md** — - **2026-06-17 (h)** — BUILT (adversarial-gaming loop, the moat): the (g) residual is CLOSED at the reference layer. The derived-minter gate trusts `inputs` as provided, so an attacker could FABRICATE an authority cell (right shape, `lock.a...
