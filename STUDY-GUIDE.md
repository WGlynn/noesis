...
3. [ ] **ROADMAP.md** — - **2026-06-17 (m)** — BUILT ✅ — token-state persistence: the (l)-DECIDED `token_cells` separation, shipped and tested. **What changed (reference layer, `node/src/runtime.rs`):** (1) `Ledger` gains a SEPARATE `token_cells: Vec<Cell>` value-...
...
- [ ] CONTINUE.md — - **BUILT ✅ — the (l) decision, shipped + tested.** `Ledger` now has a SEPARATE `token_cells` value-UTXO set; `is_valid_in_ledger` resolves token inputs against it; `apply` retires consumed inputs from AND
