...
3. [ ] **ROADMAP.md** — - **2026-06-13** — RSAW tick on `finalizes_fixed`: probed the corners (horizon=0, 100% threshold, zero-weight padding, empty voters, all-zero basis). The conservative direction `!(fixed && !float)` survives every edge — NO new break; edges ...
...
- [ ] CONTINUE.md — - **Build-order step 2 of `ON-VM-FINALIZATION.md` DONE — the consensus finalize rule now runs INSIDE the VM.** New crate `onchain/finalization-typescript` (riscv64imac ELF, 153KB, first-try compile): reads the validator set + params from th...
