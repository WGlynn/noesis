...
> Node test suite: **355 passing**.

## Read in this order

1. [ ] **WHITEPAPER-FOR-DAD.md** — *A version with no math and no jargon. — Will, with JARVIS*
2. [ ] **WHITEPAPER.md** — **Will Glynn, with JARVIS** · 2026-06-11, public since 2026-06-29 · **PUBLIC — Noesis is open source, built in the open. The canonical technical paper is [`whitepaper/noesis-whitepaper.tex`](whitepaper/noesis-whitepaper.tex) (v5.3); this is...
...
- [ ] CALIBRATION-base-case-is-god-2026-07-13.md — Source: an adversarial calibration (6 lenses × 2 refuters + synthesis, 2026-07-13) of Will's design/faith frame — that Noesis's un-derivable base case may be *named* God — and of whether it is the *same object*
...
- [ ] CONTINUE.md — **PRIOR — ORDERING TIER COMPLETE + SUB-BLOCKS slices 1/2/2b + security paper SHIPPED; ABSORPTION = RE-INCLUDE (self-contained, DA worry DISSOLVED).**
...
- [ ] DECISIONS-M3-money-2026-07-15.md — - **TSS = 3 stability MECHANISMS inside ONE token (JUL)** — L1 anchor / L2 PI / L3 rebase. Money's stability layers. - **3-token economy = JUL / VIBE / CKB-native** — three FUNCTIONS (money / governance / state-rent capital). Orthogonal.
- [ ] DEPLOY-testnet.md — **Dial the PARAMETER (`genesis_bits`), never the MECHANISM (`pow_enforced`).** `ChainSpec::testnet()` keeps PoW fully in consensus (`pow_enforced = true`) so the testnet tests the REAL security model, and
- [ ] DESIGN-LAWS.md — **Statement.** Noesis is *permitted* to be complex — a mind cannot be Bitcoin-simple — but only ORGANISM-complex, never MACHINE-complex. Elegance and complexity are orthogonal: Bitcoin is elegant-and-simple; a body is elegant-and-complex; E...
- [ ] DESIGN-M3-jul-economics.md — ---
- [ ] DESIGN-authorityless-contribution-value.md — On GitHub, a contribution becomes "real" when a **maintainer merges the PR**. The merge is an **authoritative acceptance**: a human with write‑authority over the repo judges the work valuable and admits it. Downstream value (a merged commit...
- [ ] DESIGN-block-logistics-mechanism.md — Every constant below was opened and read at its real `file:line`. **The #1 trap held:** `ARCHITECTURE.md` (and the task prompt) cite `NCI` at `lib.rs:3289`; that pointer is **stale**. The real definition is at **`lib.rs:3705`** (values matc...
- [ ] DESIGN-block-logistics.md — Block logistics are **unspecified today.** Verified in code: - No block-size cap, no block-interval, no difficulty adjustment, no block-time target. The only resource bound is `Constitution.max_mempool` (DoS Bound A).
- [ ] DESIGN-committee-attested-clock.md — Companion reading: `research/bitcoins-missing-wall-clock.md` (why a chain has no clock), memory `[[noesis-time-is-read-not-faked]]` (the two-clocks separation), `DESIGN-M3-jul-economics.md` §2.1 (the VibeSwap precedents), `DESIGN-pow-work-d...
- [ ] DESIGN-convergence-fixed-point.md — **(i) OUTER — order-theoretic (standing↔value re-fold).** Carrier = the tombstone/taint set as a subset of the finite cell set, ordered by inclusion (finite ⇒ complete lattice). The re-fold is ONE monotone
...
- [ ] DESIGN-governance-authority-tiering.md — Today the first half is enforced (weight = soulbound PoM). The second half is *not*: the measurement knobs `theta_sim_q16` and `vesting_w` are governance-amendable, and TOKENOMICS names VIBE as buyable —
- [ ] DESIGN-infrastructure-incentives.md — Ethereum did not forget to pay infrastructure. It priced the wrong *kind* of good.
- [ ] DESIGN-joint-decay-damping-rate.md — Status: ✅ built (mechanism) · framing-corrected 2026-07-12 (RSAW finding: "RHO=1/φ is numerology here").
- [ ] DESIGN-jul-money-layer.md — JUL is the **money / medium-of-exchange** token: transferable, PoW-issued, energy-pegged, designed to be **spent, not hoarded.** It is one of the three separated powers (money / governance / capital) and
- [ ] DESIGN-jul-stability-tiers.md — JUL's base layer ships **TSS Layer 1 only**: energy-anchored proportional PoW reward with the Moore's-law decay (`jul::reward_with_decay`, `noesis_core::pow::moore_decay_q32`). L1 alone is **energy-pegged and
...
- [ ] DESIGN-never-halt-liveness.md — Halting must be **structurally impossible**. Failures degrade **quality** (cadence, difficulty regulation, issuance rate) — never **liveness**. No L1 pause, no "halt issuance at height H" fallback (DESIGN-M3 §5 corrected: that fallback is F...
...
- [ ] DESIGN-pow-work-dimension.md — Makes `block_work` return a block's REAL, VALIDATED mined difficulty instead of the constant `WORK_PER_BLOCK = 1`, so: - JUL issuance becomes difficulty-proportional (`jul::reward_for_work(block_work(b), jul)`,
- [ ] DESIGN-pragma-layer-amendment-coherence.md — ---
...
- [ ] DESIGN-value-oracle-seam.md — Noesis is an AI/blockchain system: contribution flows through a value function `v(S)` whose per-cell scores become **Proof-of-Mind standing**, which weights finality. The temptation is to weld the AI (how value is scored) into consensus. Th...
- [ ] DESIGN-vesting-W-and-standing-bridge.md — Two coupled facts, both verified in code:
...
- [ ] HANDOFF-2026-07-10-block-logistics-and-essential-complexity.md — Fast orientation for a fresh window. Everything below is on disk in `~/noesis/`, **uncommitted, public repo, NOT pushed** (Will's call). Nothing here is built code yet except where noted — this was a design + paper session.
...
- [ ] PIN-quorum-floor-decision.md — **Status: RESOLVED — Will ratified the recommendation ("go with yours"; frame = thousand-year play, kernel-first). WIRED: `finalizes_pos_pom` accepts `quorum_floor_bps`; `finalizes` passes `c.quorum_floor_bps`; stale T3 comment corrected; a...
...
- [ ] RESEARCH-learned-vs-from-merged-prs.md — Everything Noesis claims ultimately rests on ONE function: `v(S)`, the value of a contribution coalition. Today `v(S)` is the DESIGNED novelty proxy (temporal novelty + θ_sim similarity floor, `lib.rs` `pom_scores_with_similarity_floor`), h...
...
- [ ] STUDY-GUIDE-TOKEN-FLOW.md — Noesis separates the functions money systems usually fuse — **money, capital/state, governance** — into three *tradeable* tokens, and holds the one thing that must never be for sale — **consensus weight** —
- [ ] SYSTEM-MAP.md — | # | Layer | What | Status | Connects to | |---|---|---|---|---| | 1 | **Rulebook** | UTXO invariants: value-conservation, no-double-spend, no-spend-of-nonexistent — the rules "easy to STATE" | ✅ | 2, 4, 5, 6(sync re-validates) |
...
- [ ] anti-plutocracy-attack-surfaces-2026-07-17.md — ---
- [ ] boardy-call-2026-07-17.md — ---
- [ ] boardy-edge-cases-2026-07-17.md — ---
...
- [ ] phase1-extraction-report.md — A single pure rulebook in `node/src/runtime.rs`:
- [ ] phase2-commitment-report.md — The engagement rule is **"all accumulator/Merkle code uses audited libraries; never hand-roll crypto."** Three options were weighed on real evidence, not preference:
- [ ] phase3-zk-plan.md — **Milestone 1 guest logic — "prove one block's transition against commitments" — is written and green.** `node/src/utxo_commitment.rs`: - `UtxoCommitment::transition(spends, creates) -> TransitionWitness` — applies a block's value
- [ ] phase4-fv-plan.md — The pure rulebook is `runtime::apply_block(state, block, params) -> Result<Ledger, Violation>` (= `validate_block` + `apply_transition`, `node/src/runtime.rs`). The invariants Phase 4 must pin:
- [ ] post-log.md — liveness fire 142 (2026-07-15 06:52): nothing to do — 0 unreviewed, 2 stamped, 0 engagement pending, restock=no, top-window=no; C1 healthy (cron 3574c13d age 3.4d, single, no HALT).
- [ ] rulebook-map.md — ---
- [ ] transparency-privacy-grounding.md — Status: design note, 2026-07-12. States the structural claim that the transparency/privacy tradeoff is **factored** in Noesis rather than balanced, and pins each half to a concrete artifact. Grounded to `file:line`; verify before claiming. ...
...
- [ ] `amendment` — Pragma layer — Constitution-amendment coherence (the socket)
...
- [ ] `utxo_commitment` — Phase 2 (stateless verification): a compact, audited commitment to the UTXO (`token_cells`) set — a 32-byte root + ~KB membership / non-memb...
- [ ] `wire` — T1 slice-1: wire codec + append-only block-log persistence (node-layer serialization of a finalized `Block`; the consensus/core types stay s...
- [ ] `net` — T1 slice-2: framed TCP transport (std::net + threads, codec-agnostic length-prefixed frames)
- [ ] `gossip` — T1 slice-3: gossip — fan-out broadcast + a blake2b seen-set for loop-prevention over slice-2 transport
- [ ] `sync` — T1 slice-4: sync — a joining node pulls a peer's block log and replays it to converge to a byte-identical state
- [ ] `jul` — JUL money layer, increment 1 — the PoW-issuance core (Lever A, the production-cost anchor; `docs/DESIGN-jul-money-layer.md`)
- [ ] `reserve` — JUL money layer, increment 3 — the counter-cyclical reserve (Lever B, the short-run smoother; `docs/DESIGN-jul-money-layer.md` §3)
- [ ] `wallclock` — Wall-clock validation kernel (Phase-2 timestamp source; `docs/DESIGN-committee-attested-clock.md`)
- [ ] `liveness` — Never-halt liveness — the stall detector (`docs/DESIGN-never-halt-liveness.md`)
- [ ] `subblock` — Sub-blocks — the fast (~2 s), REVERTIBLE, contribution-gated tier UNDER the 120 s ordering blocks (T9, `internal/RESEARCH-NETWORK-CONSENSUS....
- [ ] `chainspec` — ChainSpec — the single-sourced genesis definition ("what block zero is") every honest node agrees on: the constitution (with the ratified M3...
- [ ] `rpc` — rpc — the node's live HTTP/JSON API (`GET /state`, `POST /submit`): the interface a friend or a browser frontend calls to submit a contribut...
- [ ] `store` — store — the durable, append-only block log: the chain survives a node restart by replaying the log through the same validate+apply path a jo...
- [ ] `screen` — screen — the bootstrap ingress filter (advisory, node-local): a cheap quality + originality gate at submission time that rejects trivial/low...
