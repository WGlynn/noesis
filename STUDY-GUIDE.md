...
3. [ ] **ROADMAP.md** — - **2026-06-17 (p)** — DESIGN tick (no code; PCP-gate — 3rd moat tick this session at ~415k context during an unrelated OPH-absorption marathon; `value_flow_with_own` is the highest-blast-radius moat fn (feeds v5–v8), so its surgery belongs...
...
- [ ] CONTINUE.md — - **DESIGN tick (no code; PCP-gate — 3rd moat tick this session @ ~415k context in an unrelated OPH-marathon; `value_flow_with_own` feeds v5–v8 = highest blast radius ⇒ build in fresh low-context).** - **DECISION:** group a parent's externa...
