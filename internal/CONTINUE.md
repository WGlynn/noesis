...
> mints from work like mainnet, but low difficulty ⇒ trivial CPU energy ⇒ functional-but-WORTHLESS test
> JUL by construction, ZERO consensus change. NEVER `pow_enforced=false` to save power. Full rationale
> in the `ChainSpec::testnet()` doc-comment (`node/src/chainspec.rs`).
>
> **▶ DEPLOY SEQUENCE (mostly Will's actions — provision + credentials):**
> 1. Provision a FREE ALWAYS-ON VM (Oracle Cloud Always-Free ARM = best fit; or fly.io/GCP-e2-micro free
>    tier; NOT Vercel — static only). Will's account/credentials → not autonomous.
> 2. On the VM: `git clone` + `cargo build --release -p noesis --bin noesisd`.
> 3. Run durable + public + testnet:
>    `NOESIS_NET=testnet ./target/release/noesisd --serve-api 0.0.0.0:9955 /var/lib/noesis/chain.log`
>    (systemd unit `Restart=always` for reboot-survival; open port 9955 in the VM firewall).
> 4. Share `http://<vm-ip>:9955/` — the node serves its own UI (one URL = whole app). Friends submit,
>    you watch height/JUL/PoM grow. **A live one-node public testnet at the floor.** (Local test now:
>    `./scripts/go-live.sh`, or add cloudflared for a tunnel-while-PC-on demo.)
>
> **Honest gap list (NOT blockers for the single-node deploy; do not round up):** 🔬 live P2P gossip
> (slice-5b — `--serve-api` durable node and `--listen`/`--connect` scripted-seed are still separate
> drivers; a joiner replays history but new blocks don't propagate live; the bounded next code = make
> `--listen [store]` serve the durable log via `wire::BlockLog::load`, then unify+gossip). 🔌 the 5
> on-VM binding flips (`CONTROL_BINDING_ACTIVE` runtime.rs:542 + `CONTROL_ENFORCED`/`COORDS_BOUND`/
> `REGISTRY_BINDING_ACTIVE`/`PROVENANCE_BOUND` in `onchain/*`, all false) = the go-live milestone,
> Will-gated PCP. ⚑ `genesis_bits` testnet placeholder `0x1f00_ffff` (measured mainnet value still open).
> 🟡 faucet/seed-distributes-JUL (app-layer, so friends hold JUL), TLS/hostname, connection-cap hardening.

## 🔝🔝🔝 (superseded by the deploy block above) — SUB-BLOCK DESIGN DONE (slice-2c shipped `2c3d5a1` + 2 knobs LOCKED tune-when-live) ⇒ PIVOT to GO-LIVE / the DEPLOY POLE (`LOOP-PLAN-to-golive.md`)**

> **✅ SUB-BLOCKS FINISHED (2026-07-14 eve, `2c3d5a1`):** slice-2c DI half shipped — `Block.subblock_root: Option<[u8;32]>` inert-additive header field (coinbase/pow/timestamp/bonds precedent) + `.with_subblock_root` builder + option-tagged `header_digest` binding (PoW now PROVES the absorbed soft-chain ⇒ a swapped set changes the header ⇒ replay-impossible) + `WBlock` serde-default wire codec. 1 RED test, subblock 11 green, parity/pow/wire/sync green, lib 328, 0 new clippy. HEAD==origin==`2c3d5a1`.
> **🔒 THE 2 OPEN KNOBS LOCKED AS TUNE-WHEN-LIVE (Will good-enough ruling — do NOT re-litigate):** (1) inclusion-policy (REPLACE/MERGE/PRIORITISE + capacity cap) and (2) contribution-weight standing threshold (Sybil-cost vs liveness) are testnet-pinned PARAMETERS, not launch gates — cannot be calibrated in the abstract. Remaining sub-block items (equivocation-slashing wiring 🟡, gossip/DA live-propagation network half, apply-from-root rewire, single-source the duplicated `txs_conserve`/`commit_cell` helpers, Council adversarial pass) are UX FAST-FOLLOWS, not floor blockers — chain ships at 120s = Litecoin-class WITHOUT sub-blocks wired.
> **▶ NEXT = the DEPLOY POLE (no decision-unblocked cold builds remain, CONTINUE 07-14 PM):** L6 on-VM enforcement parity + L7 genesis/chain-spec/P2P + hosted seed node + Linux/WSL2 for the zkVM receipt + `genesis_bits` GPU measurement. This is THE remaining pole to a public testnet floor (see below / `LOOP-PLAN-to-golive.md`).
>
> **🗺 L6 ON-VM PARITY MAP (scoped 2026-07-14 eve — Will picked L6 as the next front):**
> - ✅ **lock-sig control** — on-VM twin BUILT (`onchain/locksig-typescript/src/main.rs`): reconstructs the movement, recomputes the single-sourced `noesis_core::tx::tx_digest`, verifies REAL Lamport-PQ sigs per input. Remaining = the GO-LIVE FLIP only (`CONTROL_ENFORCED`/`CONTROL_BINDING_ACTIVE` false→true + real-entropy keygen + populate `auths` across every honest flow). **That flip IS the go-live milestone — high blast-radius, deploy-coupled, needs Will's explicit "we're live" + a deploy env; NOT a casual mid-session flip (PCP).**
> - ✅ **PoM intake floors** — on-VM (`onchain/pom-typescript/src/main.rs`): soulbound identity (exit 11) + semantic/entropy floor over EVERY input+output (13/14) + per-output novelty proof vs the index-root cell-dep with F1/F2/F3 codehash binding (20-23). Streaming, no-float, single-sourced `noesis_core::classify` + `floored_from_counts`.
> - ✅ **on-VM SIMILARITY FLOOR — ALREADY BUILT + TESTED (reconciled 2026-07-15, docs-lagged-code).** The θ_sim cross-cell seen-shingle enforcement DOES run on-VM: `proven_mint_value` → `floored_from_counts(...THETA_SIM_Q16...)` zeroes any produced cell whose overlap with the committed seen-set (novelty-index root, cell-dep 0) exceeds θ_sim. E2E-covered in `node/tests/ckb_vm_proven_e2e.rs` (`redundant_mint_is_denied_on_vm_exit_22`, `mostly_recycled_second_output_is_denied`, `intra_tx_double_mint_is_denied` — all exit 22). The `pom-typescript` header claiming "not claimed here" was STALE (fixed 2026-07-15). Same Q16.16 rule as node `pom_scores_with_similarity_floor_q16`.
> - ✅ **on-VM INDEX-CELL ROOT-TRANSITION type-script — SHIPPED (2026-07-15, `34ee644`).** The OTHER half of the seen-set: the FLOOR reads the root (done), and NOW `onchain/index-typescript` PROVES on-VM that the root was honestly maintained — the committed seen-root may advance old→new ONLY as a canonically-ordered chain of single-key SMT insertions, each proven against the rolling root (drop/omit/smuggle/dup/reorder/forge all rejected). Single-sourced from `noesis_core::index_rule::valid_ordered_root_transition` (drift-guarded). Codec `parse_index_batch`/`encode_index_batch` added to `noesis_core::index_rule` (capacity from REAL length, NOT the self-declared count — critical-qa caught a `with_capacity(attacker-u32)` multi-TB pre-alloc VM-trap; fixed + regression-tested). Exit 50s (0 valid · 50 invalid order/transition, one verdict/no probe · 51 malformed · 52 unbound). 9 on-VM tests green, node lib 328 green, 8 drift-guard green, ELF 0 clippy, 0 new node clippy. Recap `sessions/2026-07-15-index-typescript-onvm-root-transition.md`.
>   - 🟡 **REMAINING = deploy-coupled activation only:** `PROVENANCE_BOUND=false` today ⇒ the ELF proves the ordered-transition MATH but still reads old/new_root + coords from the batch data, not the real INPUT/OUTPUT index cells + consensus timing. The flip (source `old_root`←input cell, `new_root`←output cell, coords←header+reveals; reject any batch-declared endpoint) is the honest-pinned deploy-coupled path — same posture as ordering `COORDS_BOUND` / finalization registry binding. Needs the index-cell + commit-reveal plumbing live.
>   - ✅ **PREREQ (2026-07-15, `fb1a4c7`):** `index_rule` single-sourced into `noesis_core::index_rule` + node re-export + 8 drift-guard tests (through the maintainer-side `NoveltyIndex` producer, node-only by design).
> - 🟡 **registry binding** (`REGISTRY_BINDING_ACTIVE` false, on-VM finalization) + **on-VM single-use / nullifier crypto** — the other L6 parity fronts, deploy-coupled-leaning.

### (historical) TOP PRIORITY (Will 2026-07-14 eve, now SATISFIED) — FINISH the sub-block design, then PIVOT to GO-LIVE

> **GOOD-ENOUGH RULING (Will, reflective — capture, do NOT re-litigate):** the sub-block DESIGN is DONE (spec + reference-built + tested + security-analyzed). The two open ⚑ (inclusion-policy, standing-threshold) are **TUNE-WHEN-LIVE** — they cannot be calibrated in the abstract; leaving them open is correct sequencing, not perfectionism. Even with sub-blocks UNWIRED the chain ships at 120s = Litecoin-class = launchable; sub-blocks are a UX FAST-FOLLOW, not a launch gate. **North star: GET ONLINE so users show up → contribution-reward mechanics turn them into the autonomous bug-bounty/improvement engine ([[voluntary-noesis]]). A live chain is the fastest path to a better chain.** "Finish sub-blocks" next window = (a) lock the 2 knobs as tune-when-live, (b) OPTIONAL: wire the header-root field into the Block header so it's structurally in, (c) then GO-LIVE path. Do not add more sub-block polish.

**PRIOR — ORDERING TIER COMPLETE + SUB-BLOCKS slices 1/2/2b + security paper SHIPPED; ABSORPTION = RE-INCLUDE (self-contained, DA worry DISSOLVED).**

**HEAD == origin == `9723c6d` (verify). Pushed. Do-not-sweep set unchanged.**

**⚑ DECISIONS RATIFIED THIS SESSION (Will):** (1) absorption = **RE-INCLUDE, NOT commit-by-root** (SUPERSEDES the earlier commit-by-root call — Will 2026-07-14 later in session): the ordering block CARRIES the absorbed txs (self-contained/replayable) AND commits their Merkle root in the HEADER (Bitcoin-standard). Decisive point: those txs would be in the 2-min block anyway ⇒ re-include is NOT heavier than baseline; the only cost is one extra fast-tier propagation round (the price of early confirmation), recovered by BIP152-style compact blocks over 6-byte `weak_tx_id`s. **⇒ DA worry DISSOLVED (block=data); NO DA-store / reject-on-unavailable to build.** `absorb()` (re-include into token_txs) + `subblock_txs_root()` (now a HEADER commitment, not the block's substitute) are both still correct — the earlier `307d6ff` commit-by-root framing is SUPERSEDED, code unchanged. (2) **ZK is NOT a DA substitute** — validity≠availability (future COMPUTE overlay via `utxo_commitment`, receipt Linux-blocked; `[[validity-not-availability]]`). (3) sub-blocks = **value txs ONLY** (contributions feed the path-dependent/gameable novelty+PoM surface ⇒ soft-confirming them is pointless-or-dangerous; security-sound reason). (4) inclusion-policy (replace/merge/prioritise mempool) still OPEN ⚑. Clock/genesis numbers ratified (δ=120s, max_staleness=300s, max-forward-skew=7200s, ASERT half-life=2d; genesis_bits + reward_den=W_g delegated/measured-at-build).

**Shipped this session:**
- **inc-CLK-2 clock enforcement** (`4e52ae6`) — the deterministic bundle, flag-gated `Constitution.clock_enforced` (default inert): `header_digest` binds the timestamp + `validate_block` enforces PRESENT + NON-DECREASING timestamp (`clock_check`, single-sourced from `wallclock::advances_monotonically` — `≥` not strict; RECONCILE CATCH: first draft used strict `>` contradicting the kernel's deliberate same-tick contract) + `Node::admits(b, local_now, δ)` node-local forward-skew bound (kept OUT of replay = Bitcoin MTP/future-time split) + genesis gate `clock_enforced ⇒ pow_enforced`. `Ledger.last_timestamp` (excluded from digest). 6 RED tests, 328 lib + all parity green, 0 new clippy. Doc: DESIGN-committee-attested-clock §8.
- **Emission ramp DROPPED** (`4a8b6b9`) — **Will correction (load-bearing tokenomics):** JUL is ELASTIC energy-money ⇒ constant JUL/energy ⇒ NO deep-capital problem (that's inelastic/BTC-specific) ⇒ NO emission ramp. A ramp would BREAK energy-peg proportionality (identical work must mint identical JUL). Flat proportional `reward_for_work` (already built) is correct + final. Corrected DESIGN-M3 §0.1/§5/§7. Memory `[[jul-elasticity-dissolves-deep-capital]]`.
- **M2 genesis JUL issuance = MECHANISM-COMPLETE (reconciled):** flat proportional reward + coinbase settlement (jul inc-1/2) + `block_work` real-difficulty under `pow_enforced` (M2a-2) + `pow_enforcement::enforced_issuance_scales_with_mined_work` ALL BUILT. No ramp to add. Remaining M2 = build-time-measured (`genesis_bits` GPU benchmark) or deploy-coupled (chain-spec, L7) — NO decision-unblocked M2 code left.

- **SUB-BLOCKS slice-1 lean core** (`6bb63d3`) — `node/src/subblock.rs`, consensus-isolated SHADOW (jul/wallclock precedent, pure validation, no networking, no finalized-state mutation ⇒ revertible, never touches `state_digest`). `SubBlock{ordering_height, seq, producer, txs}` (VALUE txs ONLY — Will 2026-07-14 + me converged: contributions vest over `W`, nothing to fast-confirm) + `validate_sub_block` (tip-binding + sequential seq + **CONTRIBUTION-WEIGHT gate** = producer PoM standing ≥ threshold, the Ergo-`T/64`→standing re-derivation + conserve/single-use against a PROVISIONAL overlay of prior sub-blocks) + `ConfirmationTier{SoftConfirmed,Final}` + `tier_of_output`. 5 RED tests, 328 lib green, 0 new clippy. **Motivating requirement:** the 120s cadence is a gift to small nodes; sub-blocks PAY FOR it in UX ⇒ not polish.
- **Sub-block security paper** (`1d1bc67`) — `docs/research/sub-block-security.md` (ready-for-critique). Load-bearing property: sub-block compromise degrades soft-confirm UX but CANNOT break settlement (revertible-by-construction + finality-excluded). 7 threats (T1 soft-confirm double-spend … T7 standing-key compromise) + mitigations + honest ⚑/🟡/🔬 residuals.
- **SUB-BLOCKS slice-2 absorption** (`ee9d4f3`) — `subblock::absorb(accepted)` deterministically flattens an interval's accepted sub-blocks in `seq` order (receipt-order-independent ⇒ two honest absorbers agree, security-paper §6 determinism); `tier_of_output` flips SoftConfirmed→Final once the output lands in the finalized token set. 2 RED tests incl. end-to-end soft→final arc. **Built autonomously while Will AFK ([[act-on-reversible-aligned-moves]]).**
- **SUB-BLOCKS slice-2b COMMIT-BY-ROOT** (`307d6ff`) — `subblock_txs_root(accepted)` = deterministic binary Merkle root over the seq-ordered absorbed txs (leaf = tx consensus-bytes w/ header_digest's anti-malleable framing; domain-separated leaves/nodes + tx-count binding vs CVE-2012-2459; empty⇒zero root) + `verify_absorption_root`. Ordering block commits the ROOT not the bytes ⇒ light block. 3 RED tests (10 subblock total). **Reference layer only — in-header field + apply-from-root are 2c (need DA).**
- **Security paper DA update** (`093d6bb`) — A5 (data-availability assumption) + T8 (data-withholding / validium-freeze threat) added, since commit-by-root makes DA load-bearing.

**▶ NEXT — GOSSIP/DA SLICE (2c) is now the CRITICAL PATH (commit-by-root DEPENDS on it, security-paper A5/T8):** wire sub-block propagation on the T1 GossipSub layer + Ergo compact 6-byte weak-tx-id propagation + the ARCHIVAL/serving path so the root's committed txs stay retrievable (DA) + reject-on-unavailable at absorption + the in-header root field + apply-from-root state transition. Decision-unblocked to START (the propagation/archival core), though the reject-on-unavailable enforcement is where DA design lives. Autonomous-buildable core; surface forks when hit.

**Still ⚑ Will:** (1) inclusion-policy — ordering block REPLACE mempool w/ soft-chain vs MERGE vs PRIORITISE + capacity cap. (2) contribution-weight threshold calibration (Sybil-cost vs liveness, testnet-pinned). **🟡:** equivocation-slashing wiring (needs persistent standing registry) · apply-from-root rewire (2c). **🔬:** merchant "wait N" quantification · ZK proof-of-absorption (compute overlay, receipt Linux-blocked). **Hardening:** Council/critical-qa adversarial pass on the sub-block tier; ✅ `txs_conserve_and_single_use` SINGLE-SOURCED onto `runtime` (2026-07-15 `0ae5a1a` — closed a real fast-tier/settlement drift: sub-block copy was missing the JUL-conservation + coinbase-squat checks, so the fast tier would soft-confirm a guaranteed-revert; RED-first, settlement byte-identical). REMAINING hygiene = single-source the `commit_cell`/`tx_commit` helpers too (byte-identical dup of `runtime::commit_cell_identity` + `header_digest`'s inline token-tx framing — pure refactor, zero behavior change; left as a clean follow-up rather than touch the consensus-critical header path late-session).

## 🔝🔝🔝 PRIOR SESSION (2026-07-14 PM/full-auto) — never-halt kernel + inc-CLK-1 + **L5 Bound-B commit-deposit** all SHIPPED via THE LOOP; Council caught real bugs in each; next = inc-CLK-2 (⚑) ∨ whitepaper ∨ M-track genesis

**HEAD == origin == `84f432c` (verify). Pushed. Do-not-sweep set unchanged (CONTINUE.md, LOOP-PLAN, finalization-typescript/main.rs, locksig/Cargo.lock, doc-coherence.py+.pyc, sessions/*.md untracked).**

**Shipped this session (all full THE LOOP — 2 planners → build → stress → Council → Pragma):**
- **never-halt liveness stall detector** (`697fc30`) — `node/src/liveness.rs`: `stalled()` + `liveness_bits()`. Pure shadow kernel, 6 RED tests, `docs/DESIGN-never-halt-liveness.md`. `max_interval` ⚑.
- **inc-CLK-1 block timestamp** (`32b87b5`) — `Block.timestamp: Option<u64>` on wire + `timestamp_admissible` helper + retarget seam test. INERT ADDITIVE (M2a-1 precedent): no consensus reads/enforces it. 6 RED tests. **Council caught:** monotonicity shipped live w/o its magnitude guard ⇒ `Some(u64::MAX)` bricks the channel; fixed by collapsing to inert data (interdependent-enforcement lesson). Recap `sessions/2026-07-14-inc-clk-1.md`.
- **L5 Bound-B commit-deposit** (`84f432c`) — the DoS economic teeth. `Constitution.submission_deposit` (default 0=inert) + `Block.bonds` + `Bond` + `check_bonds` + refund/burn resolve in `apply_transition`. Bond = real live JUL cell (full-id, is_jul, funded, authorized, single-use disjoint from token inputs); refund=leave live, burn=retire exactly one instance ⇒ conservation by construction. 8 RED tests (`node/tests/submission_deposit.rs`), 325 lib green. **Council caught 2:** (1) burn `retain` remove-all ⇒ 2×d over-burn on an identity-clone → fixed to remove-one; (2) active deposit burns cells so it must not precede its auth-guard → `Node::new` gates `submission_deposit>0` behind `CONTROL_BINDING_ACTIVE` (the CLK-1 lesson again). Docs bumped 🟡→✅ (RESOURCE-DOS-BOUNDING.md, SECURITY.md). Recap `sessions/2026-07-14-L5-bound-b-commit-deposit.md`.

**▶ NEXT (Will-pick) — DECISION-UNBLOCKED COLD BUILDS ARE DRAINED (verified 2026-07-14 PM: L5 ✓, governance ✓, L4 doc-coherence already-fixed).** What remains: (a) **inc-CLK-2 — clock ENFORCEMENT bundle as ONE unit** (monotonicity + admission INGRESS at vote path + header binding + live `next_target`). Deploy-coupled; **⚑ needs Will**: δ, max-forward-skew, genesis bits, interval, half-life. (b) **M2 genesis JUL issuance** (⚑-G settled; **⚑ genesis bits + emission**). (c) never-halt Phase-2 wiring (⚑). (d) **whitepaper** — ✅ v0.2 DRAFTED this session (`docs/research/contribution-measurement-dissolves-redistribution.md`, marginal-contribution loop, capture-resistance section from today's token work; SEED superseded) — further passes/sections are autonomous no-⚑. (e) L6/L7 deploy pole (infra). **The roadmap is now gated on a Will ⚑-ratification (unblocks CLK-2/M2) or the deploy pole — the autonomous queue is the whitepaper.** Memory this session: [[role-conflation-is-the-bottleneck]] + [[interdependent-enforcement-ships-together]] (Council validated the loop twice).

**✅ GOVERNANCE AUTHORITY TIERING — SHIPPED (2026-07-14 PM, `9ac955a` + `92f92b1`):** `docs/DESIGN-governance-authority-tiering.md` (full thread folded in) + WIRED in `node/src/amendment.rs`: `Authority {Contribution, Stewardship}` classification (inert attach-point) + the number-free **`pos ≤ pom`** anti-plutocracy mix bound (`ObligationBreach::CapitalOutweighsContribution`). 3 RED tests, lib 328 green. Model: **Tier 1** (measure: θ_sim, vesting_W, mix-direction, dimension-set) → soulbound PoM authority; **Tier 2** (dials: threshold/quorum/mempool/decay) → buyable VIBE; **all** behind the axiom gate (the real backstop). Key resolutions this thread: (1) **VIBE earned by VALIDATION** (⚑ proposal — was unspecified; fills the node-operator incentive vacuum, scoped to validators not self-solving full nodes; completes the earn-by-activity table). (2) **PoM = the shared sybil-resistance substrate** ⇒ tiering makes concave voting OPTIONAL (Tier-1 unsplittable-soulbound, Tier-2 outcome-bounded); bribery is the residual open problem. (3) anti-plutocracy is STRUCTURAL ⇒ what's left is a derived parameter pass; the learned-v(S) moat is NOT a parameter. Also shipped: **`internal/STUDY-GUIDE-TOKEN-FLOW.md`** (Will-facing, Q&A). **STILL DEFERRED:** vote-counting layer (needs VIBE token + vote curve = MD-paper); a possible `pom ≥ pow` bound (separate energy-security question, flagged not built). **Owed (Will-greenlit):** reconcile stale `[[project_nci-three-token-plan]]` (VIBE=PoM, superseded). Boardy DM (in chat) verified correct per TOKENOMICS.md.

## 🔝🔝🔝 SUPERSEDED (2026-07-14 AM/interactive) — M3 pow-arithmetic COMPLETE + timestamp-source (committee-attested clock) SPEC+KERNEL shipped; next = never-halt liveness ∨ Phase-2 wiring ∨ whitepaper

**HEAD == origin == `e4e35af` (verify). Pushed. Do-not-sweep set unchanged (CONTINUE.md, LOOP-PLAN, finalization-typescript/main.rs, locksig/Cargo.lock, doc-coherence.py+.pyc, sessions/2026-07-13*.md untracked).**

**Shipped `6184ac9`+`e4e35af` — committee-attested wall-clock (the Phase-2 timestamp source that feeds inc-M3-1's `observed` seam), ran THE LOOP solo (plan→build→self-adversarial→confluence→commit):**
- `docs/DESIGN-committee-attested-clock.md` (ready-for-critique). Trust model settled through 4 Will refinements this session: **as-trustless-as-a-double-spend** (attesting set IS the chain's own bonded finalizer set — no new trust body; every node an independent witness validating vs its OWN clock; failure mode IDENTICAL to consensus failure). Mechanism = **optimistic/challenge-on-deviation** (leaner than continuous BLS-median): proposer stamps t → every node checks |t−local|≤δ → gossip only on deviation → adjudicate by BONDED STAKE not node-count → feed observed=now−anchor. Blast-radius fence: t feeds ONLY difficulty (`FINALITY_MIX.pow==0`) ⇒ total compromise degrades cadence never safety; optional-by-construction (Option seam) ⇒ never-halt.
- **Will's epistemic frame (now the Noesis-wide CLAIM DISCIPLINE — memory [[pow-is-only-cryptographic-truth-validity-vs-veracity]]):** PoW is the ONLY on-chain self-evident truth; everything else IS the oracle problem. PoW anchors OBJECTIVE reality but for on-chain data (ordering/Sybil/issuance) not off-chain. 3-layer epistemics (L0 self-evident PoW / L1 on-chain incentive-settled / L2 off-chain imported). **Validity ≠ Veracity** (the Chainlink/Nazarov error) — market trust-MINIMIZED never "cryptographic truth". Pragma helps the MECHANISM's veracity (confluence of adjudication + Reconcile + axiom-preserving amendments via the built socket) but CANNOT give the external fact crypto-veracity (bounded, 🔬/partner, §6.1).
- `node/src/wallclock.rs` — pure validation kernel (consensus-isolated shadow, jul/reserve precedent): `within_tolerance` (node-local ±δ), `advances_monotonically` (replay-safe), `observed_elapsed` (checked_sub). 4 RED-first tests, 0 new clippy, additive. 🟡 Phase-2 wiring (block timestamp field + gossip/challenge + stake-adjudication + slashing) NOT built. ⚑ δ + max_staleness numbers open. Recap: `sessions/2026-07-14-committee-attested-clock.md`.

**▶ NEXT (Will-pick):** (a) **never-halt liveness mechanism** — min-difficulty floor + committee stall-detector (gossiped committee clocks that keep ticking through a halt); now the natural downstream of the clock (the stall-detector reuses the same committee). 🔬 designed-not-built. (b) **Phase-2 clock wiring** — block `timestamp` field + validate_block admission + deviation-challenge + stake-adjudication + slashing (deploy-coupled, needs ⚑ δ/max_staleness). (c) **draft the whitepaper** (contribution-measurement-dissolves-redistribution SEED, marginal-contribution loop — fully autonomous-buildable, no ⚑). (d) `bitcoins-missing-wall-clock.md` is now backed by a full design spec — could pair them for a publication.

**▶ OLDER M3 STATE (still true):** all four decision-unblocked pow-arithmetic increments DONE (inc-M3-0/1/2/3). Session commits: `81d244a` (M3-3 adversarial fix) · `62a0f4b` (M3-2 ceiling) · `cc3254f` (M3-1 ASERT) · `6184ac9`+`e4e35af` (committee clock).

**Shipped `cc3254f` — inc-M3-1 ASERT difficulty retarget (DESIGN-M3 §3), the last pow-arithmetic gap:**
- `noesis_core::pow::next_target(anchor_bits, height_delta, observed: Option<u64>, RetargetParams{ideal_interval, half_life, pow_limit_bits}) -> Option<u32>`. Negative-solvetime-tolerant ASERT, integer-only/float-free. EXPECTED (schedule) half live; OBSERVED half behind the `Option` seam (`None` ⇒ exactly-on-schedule ⇒ target unchanged ⇒ ships inert, Phase-1). Faster⇒harder, slower⇒easier, clamped to pow_limit floor (never-halt) + min-target 1 (never zero). BCH aserti3-2d cubic for 2^frac; two's-complement floor/frac decomp for negatives. Added integer helpers (mul_small256 overflow-saturate, 1-bit shl/shr).
- **NO economic number hard-coded** — interval/τ/floor all caller-supplied ⇒ fully decision-unblocked; ⚑ numbers are the caller's later choice. 6 RED-first tests incl EXACT power-of-two anchors (+τ⇒×2, −τ⇒÷2, frac==0). pow_arithmetic 15/15, 0 new clippy. Purely additive to noesis-core (only consumer = the test; NOT wired into consensus — that's Phase-2). Recap: `sessions/2026-07-14-inc-m3-1-asert-retarget.md`.

**▶ M3 STATE: all four decision-unblocked pow-arithmetic increments DONE** (inc-M3-0 `target_to_compact` ✓, inc-M3-1 `next_target` ✓, inc-M3-2 work-clock ceiling ✓, inc-M3-3 coinbase split ✓ + adversarial-verified). The pure arithmetic + gates are complete. **What remains is ⚑/design-heavy, NOT pure-mechanical — pick with Will:** (a) **Phase-2 wiring** of `next_target` into consensus — needs the ⚑ timestamp-source decision (bonded-committee BLS-median wall-clock per §2/§2.1) + genesis bits + τ/interval numbers; (b) **committee-attested-clock spec** (Phase-2 + L7, design-heavy, new mechanism); (c) **never-halt liveness mechanism** (min-difficulty floor + committee stall-detector, 🔬 designed-not-built); (d) **draft the queued whitepaper** (contribution-measurement-dissolves-redistribution SEED, marginal-contribution loop). This is a natural seam — the lean autonomous build fronts are shipped.

**Shipped `81d244a` — the OWED inc-M3-3 Council/adversarial pass (solo analysis; execute-to-verify, no fleet for a 1-finding surface):**
- ONE genuine finding: `Constitution.coinbase_split` was UNBOUNDED, but `jul::coinbase_slice_id` masks `index & 0xff` ⇒ a >256-entry split silently wraps (index 256 ≡ index 0) and mints two coinbase cells sharing an id — breaks "one id per (height,index)". Conservation still held (`.min(remaining)` caps total ⇒ NOT inflation; it's id-collision). Severity: genesis-config hardening, pre-launch, not a live/deployed exploit; `coinbase_split` is genesis-only (no amendable GovField).
- FIX (never-halt-aligned): `jul::MAX_COINBASE_SPLIT = 256` + `assert!` in `Node::new` (fail-loud admission) + `debug_assert!` on the pure apply path. Tests: 256-boundary distinct-ids + >256 rejected (`should_panic`).

**Shipped `62a0f4b` — inc-M3-2 work-clock ceiling (DESIGN-M3 §5):**
- `Constitution.work_clock_ceiling: u64` (default `u64::MAX` ⇒ inert/byte-identical). Apply loop clamps the CLOCK advance to `block_work.min(ceiling)`; the coinbase REWARD stays UNCLAMPED (full JUL for a hard block; clamping reward would under-pay energy / soft-cap emission). Clamp, never REJECT (rejecting on excess difficulty = censorship knob).
- GATE (same never-halt pattern as inc-M3-3): a FINITE ceiling is a genesis-admission precondition of BOTH `pow_enforced` and `vesting_w > 0` (infinite ceiling under either ships the vesting-collapse attack live). `assert!` in `Node::new`; 4 existing vesting tests updated to set a finite non-clamping ceiling.
- The ceiling VALUE (`K · expected_work`, jointly calibrated w/ `vesting_w`) is deferred ⚑ to the Phase-2 controller — this shipped the mechanism + gate only. Tests: clock-clamped-but-full-reward on a HARDER mined block + pow_enforced/vesting_w infinite-ceiling rejected-at-genesis (`should_panic`). Lib 325, pow_enforcement 6, all parity green, 0 new clippy.

**▶ NEXT M3:** (3) **inc-M3-1 ASERT difficulty controller** (prereq inc-M3-0 ✓ `target_to_compact`) ← resume here. ⚠ Carries ⚑ Will-gated numbers (genesis bits LOW-but-not-trivial per §5, retarget cadence, the `K` work-clock-ceiling calibration now unblocked-to-wire since inc-M3-2 shipped the field). Not a pure-mechanical increment — plan the calibration surface with Will before building, OR build the ASERT algorithm decision-unblocked with ⚑ numbers left as inert/placeholder + a Will-ratify gate. (4) committee-attested-clock spec (Phase-2 + L7). (5) never-halt liveness mechanism (min-difficulty floor + stall-detector, 🔬 designed-not-built). Human recaps: `sessions/2026-07-14-inc-m3-3-adversarial-verify.md` + `sessions/2026-07-14-inc-m3-2-work-clock-ceiling.md`.

## 🔝🔝🔝 NEXT SESSION (2026-07-13 PM-5, interactive w/ Will) — M3 JUL-economics: DESIGN shipped + inc-M3-0 & inc-M3-3 BUILT + NEVER-HALT invariant + Pragma case-study mission

**HEAD == origin == `84e3c85` (verify). Tree clean EXCEPT the pre-existing do-not-sweep set (LOOP-PLAN, finalization-typescript/main.rs, locksig/Cargo.lock, doc-coherence.py + .pyc — do NOT sweep).**

**Shipped + pushed this session (M3 build):**
- **inc-M3-0** (`cd915de`) — `noesis_core::pow::target_to_compact` (strict inverse of `compact_to_target`; the encoder the M3 retarget controller needs to emit candidate `bits`). Pure/total; 9/9 `pow_arithmetic` green (3 new), 0 new clippy.
- **inc-M3-3** (`6df23e7`) — N-way governable coinbase split (`Constitution.coinbase_split: Vec<(Script,u16)>`; L-INFRA carve-out). Producer takes remainder; empty default ⇒ byte-identical (`apply_block_parity` green). Conserves by construction; Σbps>10000 capped per-slice ⇒ NEVER over-mints. New `jul::coinbase_slice_id` + `SPLIT_SLICE_BIT`. `Constitution` lost `Copy`. `jul_settlement` 11/11 green. **⚠ OPEN: adversarial/council verify pass NOT run — committed on RED-first coverage; FIRST TICK next session.**
- **docs** (`84e3c85`) — `docs/DESIGN-M3-jul-economics.md` (full M3 spec, adversarially hardened) + `docs/research/bitcoins-missing-wall-clock.md`.

**KEY DECISIONS (with why):**
- **Timestamp fork RESOLVED** (DESIGN-M3 §2): Phase-1 = (c) fixed genesis difficulty, no retarget; Phase-2 mainnet = (a) **bonded-committee BLS-median wall-clock**, staleness-bounded. Branch-(b) "PoS-heartbeat" REJECTED as a category error (`last_heartbeat` is work-denominated, `lib.rs:3839`). Grounded by [[noesis-time-is-read-not-faked]] + VibeSwap whose-clock precedents (§2.1).
- **3 ⚑ numbers RATIFIED by Will-delegation** (DESIGN-M3 §0.1): fork above; `genesis_bits`=LOW (one modest GPU × generous interval; exact bits measured at build); `reward_den = W_g` ⇒ 1 JUL ≡ one genesis-block of energy, ramp ON.
- **NEVER-HALT invariant** ([[noesis-never-halt-chain]]): halting must be STRUCTURALLY IMPOSSIBLE; failures degrade QUALITY not LIVENESS; no L1 pause. Corrected DESIGN-M3 §5 (the "halt issuance at H" fallback is FORBIDDEN ⇒ degrade to min-difficulty floor). Standing design lens on every mechanism.

**▶ NEXT M3 (all decision-unblocked; THE LOOP w/ read-only `Explore` council):** (1) inc-M3-3 adversarial verify (owed). (2) inc-M3-2 work-clock ceiling (clamp clock-ONLY, precondition of `pow_enforced`). (3) inc-M3-1 ASERT controller (prereq inc-M3-0 ✓). (4) committee-attested-clock spec (Phase-2 + L7). (5) never-halt liveness mechanism (min-difficulty floor + committee stall-detector — 🔬 designed-not-built).

**⭐ PARALLEL MISSION — Pragma Confluence case study (Tom Lindeman asked for a postable X/LinkedIn study of Confluence's marginal utility in our loop):** DRAFT READY at `~/Desktop/pragma-confluence-case-study-jul-smuggle-2026-07-13.md`. Grounded in the AUTHORITATIVE site (Will-pasted; saved to [[pragma-coherence-authoritative-taxonomy]]). **Load-bearing honesty finding:** Pragma "confluence" = cohomological reading of term-rewriting confluence (H¹ obstruction to a global section); Coherence = CROSS-component failure. Our rigorous matches = Noesis convergence (= their Confluence-Conditional Schedule Independence theorem) + the amendment socket (= Protocol Confluence). **FINAL DRAFT (case-study format, coherence-reviewed 2026-07-13).** The `unified_slash` cross-path double-slash = the SOLE unambiguous confluence catch (flagship; verified at source — an overlap identity that is BOTH a collusion-ring member AND a certifier of the refuted target is slashed twice for the same harm; naive-sum over-burns mint↔sink; fix = `max` on the overlap; Klarna-family; anti-theater `max→+` reds). JUL smuggle + amount-binding = the SAME shape (a check trusts a DECLARED value ¬ the actual object) ⇒ ONE borderline finding = candidate WRONG_MAP, present for Tom to classify ¬ assert. Coherence-review FIXED the prior draft's inconsistent split (it had classed the two same-shape bugs differently). Through-line added: local-view-vs-global-truth; coherence-specific ⟺ rules genuinely COMPOSE. **VERACITY FIX (2026-07-13):** double-slash = Confluent+Axiom-breaking DANGER CELL (Tom's 2-axis, "nobody checks"), NOT non-confluent/critical-pair — naive-sum is order-independent so every replica agrees on the wrong number; Klarna (non-confluent) = the NEIGHBOR cell. **Verify+hunt workflow `wrf0l0e5w` DONE (7 agents, 2026-07-13 22:03):** all 3 verify-lenses ship-ready — Pragma-site claims verbatim-confirmed, Noesis-source claims confirmed at file:line (double-slash fix `lib.rs:4829`, anti-theater test `:5329`). TOM-AUTHOR LENS fixes APPLIED: dropped mislabeled "H1 WRONG_MAP" + "confluent" from the single-rule Finding 2 ⇒ reframed as "the boundary / framework-territory, no Coherence class claimed"; named the 2-axis grid (Confluent? × Axiom-preserving?) for the Klarna parallel; attributed the H1 statement to Pragma's GF(2)/H1 kernel; moved the "we run it by hand ¬ the engine" disclosure UP to Context; amendment-socket = attachment-point ¬ Protocol-Confluence-implementation. HUNT: 3 surfaces CLEAN; the amendment "quorum_floor>threshold breaks 2/3" catch = **FALSE POSITIVE** (agent misread `basis = eff_total.max(floor)`, `lib.rs:3962` — a HIGHER floor makes finalization HARDER; safety holds for any quorum_floor since threshold≥2/3 of a basis ≥ participation). ⇒ **ONE genuine cross-rule catch (the double-slash) is the TESTED honest number**; verify-before-assert saved a bogus BFT-safety claim to Tom. Doc BULLETPROOF + ready for Will's hand. **Will's remaining TODO before posting:** confirm the double-slash two-formulas detail + the "RS2 memory-safety-seat signed the JUL clause" memory, decide JUL/Noesis naming, add Pragma/Tom/Bernhard X handles, then post (public = Will's call). Doc = CLEAN (no meta; notes in chat per Will 2026-07-13). **FINAL ADDITIONS (2026-07-13 PM):** (a) verification-stack section — UTXO ⊕ recursive-zkVM ⊕ FV as the STATE/FOLLOWED/RIGHT axes + Pragma = the 4th (rule-set-mutation coherence); honest labels: UTXO-commit ✅ · zkVM 🔬 Linux-blocked (no receipt) · Isabelle-FV ✅ (0 admitted) · Pragma-engine = the plan. (b) engine-gap line — by-hand discipline = fallible heuristic (incl. the false-positive we caught mid-audit); engine = sound + exhaustive + certified. (c) RS2 fixed → "rust-safety seat" (REAL seat, `~/.claude/council/SPEC.md:138`, pruned-as-marginal ⇒ corroborates the beat; exact verdict is session-sourced, Will-verifiable). **DOC COMPLETE + ready for Will's hand** — only JUL/Noesis naming + Pragma/Tom/Bernhard X handles left. **▶ NEXT-SESSION MISSION (Will 2026-07-13): integrating Pragma's ACTUAL ENGINE = the bigger step** (the case study is only the on-ramp). Concrete target = a Pragma machine-checked cert on a REAL Noesis amendment via the already-built amendment socket (`node/src/amendment.rs`). TERMS-FIRST (Will drives Tom + Bernhard; Tom's POC offer is standing — [[pragma-os-crys-contact]]). **INTEGRATION SPEC DRAFTED ✅** `~/Desktop/pragma-noesis-integration-spec-2026-07-13.md` (v0, prep for the terms conversation): the built socket as attachment point (typed `Amendment` + `obligations()` Socket/Pragma tags + `verify_amendment`) → the handshake (Amendment + obligations → engine → 2-axis verdict → hash-chained cert) → the LOAD-BEARING subtlety (our attribution object = Myerson-restricted, **anonymity-RELAXED** Shapley ⇒ the engine's axiom-preservation must PRESERVE the relaxation, ¬ restore symmetry; a dangerous amendment re-introduces symmetry ⇒ re-opens the Sybil hole) → hello-world target (ONE governance-scalar amendment end-to-end to a real cert) → open Qs for Tom (representation / axiom-mapping / cert-placement / bound-input-set). Honest boundary: v1 = governance-scalar amendments only (constitutional dimension-set surface is `pending` upstream). Will drives the business terms (Tom's POC offer standing). **RULE: never label single-rule soundness "confluence."** Plain lead added per Tom's "plain not jargon" register pref. **Will's TODO before posting:** verify slash-accounting + safety-seat details, decide JUL/Noesis naming, add X handles, decide hedge level (keep hedges for Tom). NOT posted — public = Will's call.

**Memories this session:** [[noesis-time-is-read-not-faked]] · [[use-what-the-species-already-built]] · [[will-dissolution-urgency-not-pride]] · [[declare-it-easy-to-conjure-easy-solutions]] · [[noesis-never-halt-chain]] · [[pragma-coherence-authoritative-taxonomy]]. Human recap: `sessions/2026-07-13-m3-jul-economics-and-never-halt.md`.

## 🔝🔝🔝 NEXT SESSION (2026-07-13 PM-4, interactive w/ Will) — M2 PoW work-dimension COMPLETE (M2a-1 + M2a-2 both SHIPPED; Council caught+fixed a REAL binding bug); + L4, L-INFRA shipped; whitepaper seed committed

**HEAD == origin == `c7819b5` (verify with git). Shipped + pushed this session (in order):**
- **Persistent-council spec + 2 papers** (`c7819b5`) — `~/.claude/council/SPEC.md` (full build spec, DESIGN-ONLY, 4 phases + Noesis-genesis bridge gated) + `docs/research/the-standing-council-measured-adversarial-review.md` (methodology, RUN #1-4 evidence) + `docs/research/first-citizens-ai-genesis-contributors.md` (thesis: AI reviewers = Noesis's first CONTRIBUTORS/wallet-holders, EARNED-not-premined; bridge designed-not-built + Will-gated). v1/ready-for-critique. Council RSI data + persistence design in [[council-roleplay-shapley-rsi-loop]]. NOTHING BUILT — spec/papers only.
- **L4** (`8695d65`) — launch-copy honesty pins (`node/tests/launch_copy_invariants.rs`: slash-burns-never-transfers, β-bounty-fence, zero-fee) + JUL-not-deferrable doc coherence (TOKENOMICS/MVP-SCOPE/ROADMAP/POM-FINALITY-TEMPORALITY corrected to Will's 2026-07-13 JUL-is-launch-required ruling).
- **L-INFRA note** (`92a08e7` + correction `07a2220`) — `docs/DESIGN-infrastructure-incentives.md`: Ethereum's infra-incentive gap = public-goods underprovision; the 2×2; Noesis's latent edges (state-rent is a demand-funded source but currently a pure SINK not a flow — verified `lib.rs:571`; infra-as-PoM-contribution; stateless-verify dissolution; heartbeat-decay `runtime.rs:47` exists but default OFF). CONSTRAINT: **M3 must reserve a routable slice for infra**, not hard-wire miners/treasury-only. Mechanism choice = ⚑ Will/M3.
- **M2a-1** (`23a90f0`) — PoW arithmetic + data model, ADDITIVE. `noesis_core::pow` (compact_to_target + work_from_target chainwork + hasher + framer, no_std) + `PowSeal{bits,nonce}` + `Block.pow: Option` + wire serde-default. Full THE LOOP (2 planners → build → Council: math cross-checked vs Bitcoin SetCompact 2816 + bigint 4006, 0 mismatches).

- **M2a-2** (`43921f9`) — PoW ENFORCEMENT wiring, flag-gated on `Constitution.pow_enforced` (default false ⇒ byte-identical to pre-M2). `header_digest(b)` + `pow_check` (7th validate_block check: PowMissing/PowUnmet) + `block_work(b,c)` returns `work_from_target` under enforcement (JUL Lever-A live) + `Violation::{PowMissing,PowUnmet}`; `validate_block` gained `&Constitution`. **NO prev-block-hash (Will-ratified, see below).** 5 enforcement tests + 6 arithmetic + 323 lib + all parity green, 0 new clippy. `docs/DESIGN-pow-work-dimension.md` shipped with it.
  - **Council `wp1ic7ed8` caught a REAL major binding bug (fixed before commit):** `header_digest` had delegated `token_txs` to the canonicalized, auth-free `TokenTx::digest` ⇒ a solved PoW replayed onto blocks with reordered token-outputs (state-divergent) or swapped `auths` (differently-authorized once `CONTROL_BINDING_ACTIVE` flips). FIX: commit each token_tx's consensus-consumed bytes in PRESENTED order (standard/code_hash/args/inputs/outputs/auths via full-cell framing) + a new binding test `header_digest_binds_token_tx_order_and_auths`. Council also fixed a weak anti-theater note (flag-off asserted at easy-work=1, indistinguishable from always-derive ⇒ now asserts harder-work=256). **PROCESS: review agents had WRITE access + mutated the shared tree to test notes; verified reverted-clean before commit — next council MUST use read-only `Explore` agents.**
- **whitepaper seed** (`666c1f2`) — `docs/research/contribution-measurement-dissolves-redistribution-SEED.md` (see below).

**NO-PREV-HASH (Will-ratified 2026-07-13, load-bearing):** Noesis deliberately does NOT hash-chain blocks. Unlike Bitcoin (PoW+chain = ordering security), Noesis PoW is finality-EXCLUDED (`FINALITY_MIX.pow=0`, reorgeable); ordering = coord-sourced canonical order + PoS+PoM finality. PoW's job here = issuance (JUL) + liveness + per-block Sybil-cost. Prev-hash would be vestigial. Will's framings: (a) "we aren't Bitcoin, complex but works"; (b) "JUL elasticity makes the 'for issuance' line hit harder — elastic energy-money vs scarce speculative asset."

**QUEUED WHITEPAPER (Will 2026-07-13, "worth its own whitepaper 100%"):** `docs/research/contribution-measurement-dissolves-redistribution-SEED.md` (committed `666c1f2`; SEED/outline only — draft with the marginal-contribution loop, not rushed). Thesis: measured contribution DISSOLVES the socialist-redistribution dichotomy — public goods (bug bounties, RPGF, infra, code) funded as payment-for-measured-value, not redistribution. Bug-bounty for fraud = already built (`dispute::resolve_refuted` + β-bounty); general value = the open moat. Extension: apps need NO bolted-on speculative token ⇒ pure-utility chain, no per-app pump-and-dump fragmentation (OSCH-for-tokens). Draft with the marginal-contribution loop, not rushed.

**▶ NEXT (resume here):** M2 + L4 + L-INFRA all shipped + pushed (HEAD `666c1f2`). Open build fronts, pick per Will: **M3** (JUL economics — ⚑ Will numbers: retarget rule + genesis bits + block interval + emission rate + work-clock ceiling; constrained by L-INFRA to reserve an infra slice) · **L-INFRA mechanism** (⚑ meter+fund choice) · **draft the whitepaper** (marginal-contribution loop) · **M1** reserve-wiring (deploy-coupled). **COUNCIL v-next (Will 2026-07-13 idea):** make council members PERSISTENT subagents with `.soul` files + per-member finding-ledgers (track-record → Shapley efficacy → add/prune) for richer roleplay + compounding — the [[council-roleplay-shapley-rsi-loop]] made a standing institution; also fix the write-access hazard (read-only `Explore` agents). **Pre-existing dirty tree (Will's, do NOT sweep):** `finalization-typescript/src/main.rs`, `scripts/doc-coherence.py`, `locksig/Cargo.lock`, `LOOP-PLAN-to-golive.md`, + this handoff `CONTINUE.md`.
...
**✅ RESOLVED — GENESIS BOOTSTRAP (Will 2026-07-13): PoW starts genesis (issuance/liveness) · bonded PoS finalizes · handoff to PoM.** Two jobs the phrase "carry genesis" was hiding, now split: **(1) PoW starts genesis** = issuance + liveness + ordering + Sybil-cost; the first coins (JUL) come from energy — permissionless, NO premine, fair-launch (the money layer). **(2) Bonded PoS finalizes genesis** — REQUIRED, not merely reasonable: `FINALITY_MIX` excludes PoW (reorgeable) and PoM=0 at t0, so genesis finality-weight is zero unless stake is injected by fiat ⇒ the bonded set is the only finalizer of block zero. As PoM accrues → mints state-bytes → PoS becomes endogenous → finality shifts to PoS+PoM → the bonded scaffold recedes. Mix is **dynamic** (PoW+bonded @ genesis → PoM-dominant steady-state). Honest correction to "PoW is optimal ∵ exogenous": the conclusion holds (PoW = fair issuance bootstrap) but the true selector is *permissionless + no-pre-allocation + credibly-neutral*, NOT exogenous-vs-endogenous (a bonded set is exogenous-by-fiat). **Open sub-params (not yet decided):** does the bonded set hold money or only authority (moneyless + decaying recommended)? · decay/handoff schedule? · a testnet must exercise the REAL PoW→PoM mainnet genesis before launch (bonded-devnet does NOT test it). Memory: `[[project_noesis-genesis-bootstrap-decision]]`.
...
- **▶ Phase 3 (NEXT) — zkVM PoC + honest cost report (human go/no-go).** ⚠ **No STARK prover on this Windows box**
  ...
  **⚑ B DEFERRED to a Linux env (Will's dev note, 2026-07-12):** the real STARK receipt + proving-cost
  ...
  measure that cost.)
- **Phase 4 — FV (2026-07-12): Steps 1 ✅ + 2 ✅ + 3 ✅ ALL MACHINE-CHECKED GREEN — Phase 4 (per-execution invariants) COMPLETE per the plan's acceptance.** Commits: `4fdc34b` (Step 1 — 11 `node/tests/fv_invariants.rs` property tests, no-dep xorshift to dodge the `dlltool` blocker) · `1eb0373` (Step 2 — spec-oracle differential vs an independent BTreeMap model, no divergence) · `6068168`+ (Step 3 — `internal/fv/Noesis_Rulebook.thy`: `conservation` + `no_double_spend` + `determinism` discharged under a **locally-installed Isabelle2025**, 0 `sorry`; reproducible via `internal/fv/verify.sh`, pinned URL+sha256; model-to-code gap G-a..G-e enumerated in the README). Isabelle install lives at `C:/Users/Will/Isabelle2025` (bundle at `C:/Users/Will/isabelle-install/`). **Remaining = SEPARATE optional 2nd-line-of-defence axis:** rule-set-mutation coherence (Pragma Confluence) on the Constitution-amendment surface — do NOT conflate with the per-execution invariants ([[pragma-confluence-governance-fv]]). Full plan:
...
1. **Resource-DoS bounding — Bound A SHIPPED ✅** (`de59148`←`b80896e`). `Constitution.max_mempool` cap; `Node::submit`
   returns `bool` + rejects admission when the pool is full — deterministic, economics-independent ceiling on mempool memory +
   per-proposal compute. Per-replica liveness/resource guard, NOT consensus-affecting (never enters `validate`; no finality
   touch). RED→GREEN `resource_dos_flood_is_bounded_by_mempool_cap`, full suite **318/318**, 0 new clippy. Design +
...
- **(qq) tx_digest serializer ported to noesis-core BUILT ✅:** `noesis_core::tx::{CellView, tx_digest}`
