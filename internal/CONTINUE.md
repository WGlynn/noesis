...
## ▶ RESUME HERE (2026-06-18 (r) — NAMED + DESIGN: multi-identity-split gap pinned; fix designed, build deferred to fresh ctx)
- **NAMED (committed 1543390):** the (q) λ^r damping closes SINGLE-identity volume, but K distinct vested identities × 1 child each → every child rank-0 → damping inert → flow climbs K1 14.28→K8 20.57 (past the 18.11 single-id bound). Test `multi_identity_split_volume_defeats_per_identity_damping_open_gap` (221/221). Root cause: `value::max_certifying_identities` (line ~851) DEFINED but NOT wired into `value_v6`.
- **FIX DESIGN (build next, FRESH low-context — production value-path change):** two options for the per-parent cross-identity bound, mirroring the within-identity λ^r —
  - **Opt A (hard cap):** in the flow accumulation, cap DISTINCT certifying identities per parent at `max_certifying_identities(Σ standing of this parent's certifiers, floor)`; identities beyond the cap (lowest-flow first) contribute 0. Structural, sharp, matches A3.
  - **Opt B (geometric, symmetric to λ^r):** sort a parent's certifying identities by their grouped contribution desc; weight the m-th identity by μ^m (μ=1/φ candidate). Soft; one identity stays full, additional identities decay — same shape as the within-identity rank decay, applied across the identity axis. Honest diverse certification (few identities) ~untouched; volume-via-many-identities saturates.
  - **Recommend B** per PONYTAIL (soft, preserves honest multi-identity certification; A can over-punish legitimately broad certification). Either way: (1) deterministic — sort identities by a canonical key (e.g. contribution then args) so replicas converge; (2) honest cases INERT (1-2 honest certifiers ⇒ ranks 0/1 ⇒ ~full weight); (3) MUST mirror in `value_flow_external_q32` (Q32.32) + hold the `v7_q32_tracks_f64` drift-guard; (4) flip the (r) open_gap test to bounded + honest-number the new curve.
- **NEXT after this:** lock-sig binding (existence→control) · on-VM single-use per (k). Then the learned-v(S)-on-real-labels mile (THE moat).
