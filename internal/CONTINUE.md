...
**🔝 CLEAN-SESSION START (Will 2026-06-22 "start clean session on next moat increments"):** `unified_slash` cross-path bound is **BUILT + edge-hardened + accounting-pinned** (ROADMAP (jj)/(kk); `dispute::unified_slash` + `refuted_lineage_identities`, lib 238→243, **suite 294 green**, extraction-audit PASS). **▶ #1 NEXT BUILD = `unified_settlement`** (ROADMAP (kk) build contract): `unified_settlement(collusion, refutation, overlap, standing) -> Settlement` emitting the merged slashes AND corrected `burned = Σ merged` (zero canceled/payout — a cross-path bound carries no new bounty), with a mint↔sink test (`Σ slashes == burned`) + an overlap test (`burned < collusion.burned + Σ refutation.slashes`). Lowest blast radius of the queue (additive, composes `unified_slash`). **THEN:** `finalizes_pos_pom` T3-wiring (preserve ref↔on-VM parity) · lock-sig DEPLOY (link crypto verifier) · 2-level recursion (P2) · **learned-v(S) on real labels = THE moat (data-blocked)**. All fresh-low-context per the locked discipline.
