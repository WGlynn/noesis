...
| **Learned v(S) moat** | 🔬 open — NULL **thrice**: DeepFunding round 1 + faithful port (95/115 ancestor coalitions singleton ⇒ topology-degenerate), and now crates.io deep-ancestry (2026-07-15, `data/crates/RESULTS.md`, `5d1a084`) where topology is EXCLUDED (ancestor-singleton 0.18) yet predictive stays null ⇒ null is ROBUST; adversarial instrument ✅ 2026-07-02 (still constructed fixture) | Launch copy = **floor only**, never "un-gameable value chain." W substitutes temporally for the moat on the finality path. **Data hunt DISCHARGED** — the deep-ancestry dataset obligation is met; the predictive null is decisive ⇒ the moat rests on the STRUCTURAL defense, not the predictive win. Still open (undated): a real *adaptive* adversary + the general graph-iso theorem. |
...
- **Warm:** moat data hunt ✅ **DONE 2026-07-15** (deep-ancestry dataset supplied via crates.io; predictive null a 3rd time, decisive; `data/crates/RESULTS.md`). TOKENOMICS NCI doc reconciliation. Week-0 spillover.
...
- 🔬 Learned-v(S) moat on real labels — the deep-ancestry dataset is now supplied (2026-07-15) and the predictive half is NULL a 3rd time, so the *moat* (adversarial-robustness on a real adaptive adversary) is still NOT delivered; what's confirmed is that it rests on the structural defense, not a predictive win.
